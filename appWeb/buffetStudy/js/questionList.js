function initDiv(){
	$.ajax({
		  ...
		  async:true,
		  dataType:"json",
		  data:{loreId:loreId,studyLogId:studyLogId,nextLoreIdArray:nextLoreIdArray,studyType:studyType,loreTypeName:escape(loreTypeName),access:access},
		  url:"buffetApp.do?action=loadQuestionList&cilentInfo=app",
		  beforeSend:function(){
        	$("#loadDataDiv").show().append("<span class='loadingIcon'></span>");
		  },
		  success:function (json){ 
			  showLoreQuestionList(json["result"]);
		  },
		  complete:function(){
        	$("#loadDataDiv").hide();
        	$(".loadingIcon").remove();
...
function showLoreQuestionList(list){
	...
	if(list != null){
		for(var i = 0 ; i < questionLength; i++){
			...
			//题号,题类型,题目标题
			var question_typeList_dd = "<div id='loreType_"+index+"' class='loreTypeTit clearfix'><span class='loreTypeNum fl'>"+index+"</span><span class='loreTypeTxt fl'>"+list[i].loreQuestion.questionType+"<em></em><i></i></span><div class='quesTit fl'>"+list[i].loreQuestion.subject+"</div></div>";
			...
			submit_div += "<span class='goNextBtn removeAFocBg' ontouchend=submitAnswer("+ currentLoreId +","+index+","+answerNumber+","+studyLogId+",'"+encodeURIComponent(encodeURIComponent(answer_array))+"','"+list[i].loreQuestion.questionType+"')>提交</span>";
			submit_div += "</div>";
	...
	if(lastCommitNumber == questionLength){
		if(loreTypeName == "再次诊断"){
			...
			if(currentAllQuestionFlag == 1){//表示当前题型全部正确
				...
				//step=0:表示不修改step的值
				if(checkFirstStepLore(allLorePath,currentLoreId)){//当第一级关联知识点一次完全通过时
					studyLogId_curr = updateLogStatus(4,0,1,1,currentLoreId);
				}else{
				...
				if(checkLoreId(allLorePath,currentLoreId)){
		...
		var url = "&buffetId="+buffetId+"&buffetName="+buffetName+"&bsdId="+buffetStudyDetailId+"&basicLoreId="+basicLoreId;
		url += "&tracebackType=review"+"&studyLogId="+studyLogId_curr+"&closeFlag=tp";
...
function checkFirstStepLore(lorePath,currentLoreId){
	var flag = false;
	var pathArray = lorePath.split(":");
...
function updateLogStatus(step,stepComplete,isFinish,access,currentLoreId){
	...
	$.ajax({
		  ...
		  async:false,
		  dataType:"json",
		  data:{bsdId:buffetStudyDetailId,studyLogId:studyLogId,loreId:loreId,step:step,stepComplete:stepComplete,
			  	isFinish:isFinish,access:access,loreTaskName:escape(loreTaskName),currentLoreId:currentLoreId,
			  	currentStepLoreArray:currentStepLoreArray
		  },
		  url:"buffetApp.do?action=updateLogStatus&cilentInfo=app",
...
function excuteInsertData(loreId,currentLoreId,studyLogId,loreType,questionId,questionStep,myAnswer,answerOptionArray,questionType){
	$.ajax({
		  ...
		  async:false,
		  dataType:"json",
		  data:{loreId:loreId,currentLoreId:currentLoreId,studyLogId:studyLogId,loreType:escape(loreType),
			  loreQuestionId:questionId, questionStep:questionStep,myAnswer:myAnswer,
			  buffetId:buffetId,bsdId:buffetStudyDetailId,loreTaskName:escape(loreTaskName)
		  },
		  url:"buffetApp.do?action=insertStudyInfo&answerOptionArray="+answerOptionArray+"&cilentInfo=app",
		  success:function (json){
			  if(json["result"] == 0){//错
				  ...
				  $("#infoTxt").html("哎呀，答错了！").css({"padding-top":18});
				  $("#goldenTxt").html("");
				  currentAllQuestionFlag *= 0;
				  ...
				  $("#infoTxt").html("恭喜你，回答正确！").css({"padding-top":18});
