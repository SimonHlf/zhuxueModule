...
function getStepStudyInfo(){
	...
	$.ajax({
		  ...
		  url:"buffetApp.do?action=getStepStudyJson&cilentInfo=app",
	...
	if(list != null){
		...
		for(var i = 0 ; i < questionLength; i++){
			...
			if(list[i].realAnser != ""){//做过的题
				...
				//$("#answer_option_dd_"+index).hide();//隐藏答案选项
				//$("#result_answer_dd_"+index).show();//显示我的解答
				//列出答案和我的答案
...
function excuteInsertData(loreId,currentLoreId,studyLogId,loreType,questionId,questionStep,myAnswer,answerOptionArray,questionType){
	$.ajax({
		  ...
		  dataType:"json",
		  data:{
			  loreId:loreId,currentLoreId:currentLoreId,studyLogId:studyLogId,loreType:loreType,
			  loreQuestionId:questionId,questionStep:questionStep,
			  myAnswer:myAnswer,loreTaskName:escape(loreTaskName),buffetId:buffetId,bsdId:buffetStudyDetailId
		  },
...
function lastSubmitAnswer(currentLoreId){
	...
	if(lastCommitNumber == questionLength){
		...
		studyLogId_curr = updateLogStatus(step_new,0,1,3,currentLoreId);	
		var url = "&bsdId="+buffetStudyDetailId+"&buffetId="+buffetId+"&basicLoreId="+basicLoreId+"&nextLoreIdArray="+nextLoreIdArray;
		url += "&tracebackType=review"+"&buffetName="+buffetName+"&studyLogId="+studyLogId_curr+"&closeFlag=tp&studyType="+studyType;
...
function updateLogStatus(step,stepComplete,isFinish,access,currentLoreId){
	...
	$.ajax({
		  ...
		  dataType:"json",
		  data:{
			  bsdId:buffetStudyDetailId,studyLogId:studyLogId,loreId:loreId,step:step,
			  stepComplete:stepComplete,isFinish:isFinish, access:access,
			  loreTaskName:escape(loreTaskName),currentLoreId:currentLoreId,type:"study"
		  },
...
//返回上一页
function backPage(){
	var url = "&bsdId="+buffetStudyDetailId+"&relateLoreIdStr="+relateLoreIdStr+"&basicLoreId="+basicLoreId;
	url += "&basicLoreName="+encodeURIComponent(encodeURIComponent(basicLoreName))+"&buffetId="+buffetId;
	url += "&buffetName="+encodeURIComponent(encodeURIComponent(buffetName))+"&buffetSendId="+buffetSendId+"&closeFlag=mp";
